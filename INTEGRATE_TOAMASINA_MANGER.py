...
# -*- coding: utf-8 -*-
"""
🎯 INTÉGRATION SÉCURISÉE TOAMASINA - Phase 1: MANGER
Intègre les 15 fiches restaurants dans lieux.js de manière sécurisée.
"""
...
NEW_DATA_FILE = PROJECT_ROOT / 'toamasina_manger.json'
...
def validate_location(loc):
    ...
    if 'manger' not in loc['tags']:
        return False, "Tag 'manger' manquant"
...
def integrate_locations():
    ...
    print("🎯 INTÉGRATION TOAMASINA - PHASE 1: MANGER")
    print("=" * 80)
    ...
    for i, loc in enumerate(new_locations):
        loc['id'] = next_id + i
        # Ajouter champ image par défaut
        if 'image' not in loc:
            nom_clean = loc['nom'].lower().replace(' ', '-').replace("'", '').replace('&', 'et')
            loc['image'] = f"images/toamasina/manger/{nom_clean}.jpg"
    ...
    print("\n✅ Intégration réussie !")
    print("\n🧪 Prochaine étape: Tester les filtres dans le navigateur")
