...
window.initSpotsPage = function () {
    ...
    console.log("🔍 INIT SPOT PAGE...");
    if (!window.LIEUX_DATA) {
        console.warn("❌ LIUEX_DATA missing in initSpotsPage");
        return;
    }

    // Filter logic: spotLocal = true OR type = 'Spot Local' OR tags include 'spots'
    const spots = window.LIEUX_DATA.filter(l =>
        l.spotLocal === true ||
        l.categorie === 'Spot Local' ||
        l.type === 'Spot Local' ||
        (l.tags && (l.tags.includes('spots') || l.tags.includes('spot')))
    );
    console.log(`🔍 Found ${spots.length} local spots.`);

    if (spots.length === 0) {
        spotsContainer.innerHTML = "<p style='text-align:center; padding:20px; color:var(--text-secondary);'>Aucun spot secret trouvé pour le moment (Code: 0).</p>";
    } else {
