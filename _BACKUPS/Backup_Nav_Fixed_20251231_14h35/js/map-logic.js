...
window.initMap = function () {
    ...
    // CartoDB Voyager : Fond de carte Premium, épuré et moderne
    ...
    // Track map movement to prevent hover-out close during panning
...
window.updateMapMarkers = function () {
    ...
    filtered.forEach(lieu => {
        ...
        // Survol: Ouvre + Centre (Smooth Preview)
        marker.on('mouseover', function () {
            this.openPopup();

            // Auto-Center on Hover (Requested)
            const map = window.leafletMap;
            const targetLatLng = L.latLng(lieu.lat, lieu.lng);
            const zoom = map.getZoom();
            const point = map.project(targetLatLng, zoom);
            ...
            // Only center if not already centering to avoid jitters
            if (!map._isMapMoving) {
                map.setView(newCenter, zoom, { animate: true, duration: 0.5 });
            }
        });

        // Sortie: Ferme UNIQUEMENT si non verrouillé ET si la carte ne bouge pas (pour éviter fermeture pdt anim)
        ...
        marker._lieuId = lieu.id; // Store ID for deep linking
    });
...
/**
 * Open specific marker by ID (Deep Link from Modal)
 */
window.openMapMarker = function (id) {
    if (!markersLayer) return;

    // 1. Force Reset Filters to ensure marker exists
    // We basically check everything to be safe
    document.querySelectorAll('.filter-checkbox').forEach(cb => cb.checked = true);
    document.querySelectorAll('.filter-chip-map').forEach(chip => chip.classList.add('active'));

    // 2. Re-render map to apply "Show All"
    updateMapMarkers();

    // 3. Find marker
    const layers = markersLayer.getLayers();
    // Loose equality (id might be string vs number)
    const marker = layers.find(l => l._lieuId == id);

    if (marker) {
        // Use MarkerCluster's zoomToShowLayer to handle clustering
